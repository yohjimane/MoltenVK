...
static void _encodeDrawIndirectCountICB(MVKCommandEncoder* cmdEncoder,
	...
	id<MTLArgumentEncoder> argEnc =
		encPool->getCmdDrawIndirectCountICBMTLArgumentEncoder(indexed, mtlIdxType);
...
void MVKCmdDrawIndirectCount::encode(MVKCommandEncoder* cmdEncoder) {
	...
	if (pipeline->isTessellationPipeline()) {
		...
		VkDeviceSize indirectPerDraw = sizeof(MTLStageInRegionIndirectArguments) + 2 * sizeof(MTLDispatchThreadgroupsIndirectArguments) + sizeof(MTLDrawPatchIndirectArguments);
		...
		// Convert kernel: once for all draws.
		cmdEncoder->encodeStoreActions(true);
	...
	_encodeDrawIndirectCountICB(cmdEncoder,
								...
								false, MTLIndexTypeUInt16, nil, 0);
}
...
void MVKCmdDrawIndexedIndirectCount::encode(MVKCommandEncoder* cmdEncoder) {
	...
	if (pipeline->isTessellationPipeline()) {
		...
		for (uint32_t batchStart = 0; batchStart < _maxDrawCount; batchStart += batchSize) {
			...
			for (uint32_t drawIdx = batchStart; drawIdx < batchEnd; drawIdx++) {
				...
				// Index copy (direct Metal calls to avoid state tracker interference).
	...
	_encodeDrawIndirectCountICB(cmdEncoder,
								...
								_mtlCountBuffer, _mtlCountBufferOffset,
								_maxDrawCount, _mtlIndirectBufferStride,
								true, (MTLIndexType)ibb.mtlIndexType,
								ibb.mtlBuffer, ibb.offset);
}
